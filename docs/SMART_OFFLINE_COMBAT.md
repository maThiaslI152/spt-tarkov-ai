...
> **Related:** [PERFORMANCE_ARCHITECTURE.md](PERFORMANCE_ARCHITECTURE.md) §2.5, [PERFORMANCE_PLAN.md](PERFORMANCE_PLAN.md) Phase 2.5, [PROGRESS.md](PROGRESS.md)
...
| **Materialization hook (stub)** | `Components/OfflineSquadMaterialization.cs` | `TryBeginMaterialize` always returns `false`; documents recommended **1.5×** hearing-radius pre-materialization guideline for future work. |
...
1. **Dematerialization** — Far fights are not replaced by stats-only entities; spawned bots are not removed from full simulation based on offline state alone.  
2. **Offline → online materialization** — No spawn/restore of bots, inventory, or corpses from `OfflineCombatResult` when the player enters the zone (stub only).  
...
| P2 | **Materialization** | Implement `OfflineSquadMaterialization.TryBeginMaterialize`: early spawn, `UnregisterOfflineSquad`, state handoff from last `OfflineCombatResult`. |
| P2 | **Dematerialization strategy** | Design interaction with AILimit, ABPS caps, and SAIN perception tiers so “offline” does not fight full-AI assumptions. |
