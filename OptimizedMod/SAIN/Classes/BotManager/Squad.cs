...
public class Squad
{
    ...
    public event Action<BotComponent, Enemy> OnEnemySightedByMember;
    ...
    public void AddMember(BotComponent bot)
    {
        ...
        if (bot?.Player != null && bot.BotOwner != null)
        {
            ...
            if (!Members.ContainsKey(bot.ProfileId))
            {
                ...
                // Phase 3.1: Subscribe to squad enemy detection propagation
                // When any squad member sees an enemy, all members get the detection.
                // This eliminates redundant per-bot per-body-part raycasts within squads.
                OnEnemySightedByMember += ReceiveSquadEnemyDetection;
    ...
    /// <summary>
    /// Phase 3.1: Receive shared enemy detection from a squad member.
    /// When a squadmate spots an enemy, all other members get the detection without
    /// needing to run independent per-body-part visibility raycasts.
    /// Collapses visibility from O(N^2) to O(N) for grouped bots.
    /// </summary>
    private void ReceiveSquadEnemyDetection(BotComponent spotter, Enemy enemy)
    {
        if (enemy == null || spotter == null || _isPropagatingEnemy)
            return;

        _isPropagatingEnemy = true;
        try
        {
            foreach (var member in Members.Values)
            {
                // Don't propagate to the spotter itself
                if (member == spotter)
                    continue;

                // Skip if already has this enemy
                if (member.EnemyController.IsPlayerAnEnemy(enemy.EnemyProfileId))
                    continue;

                // Add enemy as "known from squad callout" — skip raycast verification
                // since a squad member already confirmed visual contact
                var iPlayer = enemy.EnemyIPlayer;
                if (iPlayer != null)
                {
                    var squadEnemy = member.EnemyController.CheckAddEnemy(iPlayer);
                    if (squadEnemy != null)
                    {
                        // Mark as known from squad callout at the spotter's observed position
                        squadEnemy.KnownPlaces.UpdateHeardPlace(
                            enemy.EnemyPosition,
                            true,  // isDanger
                            spotter.ProfileId
                        );
        ...
        {
            _isPropagatingEnemy = false;
        }
    }

    private bool _isPropagatingEnemy;
