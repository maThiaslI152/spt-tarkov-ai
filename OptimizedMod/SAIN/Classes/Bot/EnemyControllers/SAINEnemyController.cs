...
public class SAINEnemyController : BotComponentClassBase
{
    ...
    private void EnemyAdded(Enemy enemy)
    {
        ...
        // Phase 3.1: Squad awareness — propagate enemy detection to all squad members
        // so they don't need to independently run per-body-part raycasts for the same enemy.
        // Only propagate when the enemy is actually visible (not just known from sound).
        PropagateSquadEnemyDetection(enemy);
    }

    /// <summary>
    /// When an enemy is sighted, share detection with all squad members.
    /// This eliminates redundant per-bot raycasts within squads (O(N) instead of O(N^2)).
    /// </summary>
    private void PropagateSquadEnemyDetection(Enemy enemy)
    {
        // Guard against re-entrancy from squad propagation
        if (_isPropagatingSquad || enemy.EnemyPlayer == null)
            return;

        var squadInfo = Bot.Squad.SquadInfo;
        if (squadInfo == null || squadInfo.Members == null || squadInfo.Members.Count <= 1)
            return;

        // Only propagate if the enemy is currently visible (real sight, not just sound)
        if (!enemy.IsVisible)
            return;

        _isPropagatingSquad = true;
        try
        {
            squadInfo.OnEnemySightedByMember?.Invoke(Bot, enemy);
        }
        finally
        {
            _isPropagatingSquad = false;
        }
    }

    private bool _isPropagatingSquad;
    ...
    /// <summary>
    /// Phase 4: Clear all enemies for pool recycling.
    /// Completely resets enemy tracking so the recycled bot starts fresh.
    /// </summary>
    public void ClearAllEnemies()
    {
        ...
        EnemiesInLineOfSight.Clear();
        VisibleEnemies.Clear();
        _listController.ClearAll();
        _alliesHash.Clear();
        _allyIdsToRemove.Clear();
        _invalidIdsToRemove.Clear();
        _nextEnemyUpdateTime = 0f;
    }
}
