...
internal class CombatSquadLayer(BotOwner botOwner, int priority) : SAINLayer(botOwner, priority, Name, ESAINLayer.Squad)
{
    ...
    {
        // Phase 3.3: Use cached/throttled IsActive check (State Tree pattern)
        // Only re-evaluates active layer every frame; inactive layers at 5Hz
        ...
        if (GetBotComponent())
        {
            ...
            if (bot != null && bot.BotActive)
            {
                ...
                {
                    // Phase 3.2: If this bot is the squad leader, coordinate the entire squad.
                    // Centralized coordination replaces per-member independent evaluation.
                    if (bot.Squad.IAmLeader)
                    {
