...
public class BotComponent : BotComponentBase, ISPlayer
{
    ...
    /// <summary>
    /// Player-centric perception tier — determines how much AI processing this bot gets.
    /// Visible = full AI, Audible = movement + basics, Occluded = navigation only.
    /// </summary>
    ...
    public void ManualUpdate(float currentTime, float deltaTime)
    {
        ...
        if (botOwner != null)
        {
            ...
            if (player != null)
            {
                ...
                var perceptionTier = CurrentPerceptionTier;

                // Occluded: only always-tick classes (navigation), skip everything else
                if (perceptionTier == PerceptionTier.Occluded)
                    return;
                ...
                // Audible: skip combat classes, keep movement + basics
                if (perceptionTier == PerceptionTier.Audible)
                    return;
    ...
    /// <summary>
    /// Phase 4: Full state reset when this bot is recycled from the pool.
    /// Clears all AI state so the reused GameObject behaves like a fresh spawn.
    /// Called after position/health/inventory reset by the pool system.
    /// </summary>
    public void ResetForPoolRecycle()
    {
        ...
        // Clear enemy list completely
        EnemyController.ClearAllEnemies();

        // Reset decision state
        Decision.ResetDecisions(true);

        // Reset activation state (force full reactivation)
        BotActivation.ResetActivationState();

        // Reset AI limit tracking
        AILimit.ResetForPoolRecycle();

        // Reset tick timers so all classes re-tick on next frame
        LastCheckVisibleTime = 0f;
    }
}
