...
public class CoverFinderComponent : BotComponentBase
{
    ...
    private WaitForSeconds _coverFindWait;
    private float _lastCoverFindInterval;
    ...
    private IEnumerator FindCoverLoop()
    {
        while (Bot != null)
        {
            ...
            if (_coverFindWait == null || Mathf.Abs(_lastCoverFindInterval - interval) > 0.001f)
            {
                _lastCoverFindInterval = interval;
                _coverFindWait = new WaitForSeconds(interval);
            }
