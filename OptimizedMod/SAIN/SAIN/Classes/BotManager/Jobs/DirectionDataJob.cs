...
public class DirectionDataJob : BotManagerBase
{
    ...
    private readonly List<PlayerComponent> _directionDataPlayers = [];
    private WaitForSeconds _directionDataWait;
    private float _lastDirectionDataInterval;

    private WaitForSeconds GetDirectionDataWait()
    {
        ...
        if (_directionDataWait == null || Mathf.Abs(_lastDirectionDataInterval - interval) > 0.001f)
        {
            _lastDirectionDataInterval = interval;
            _directionDataWait = new WaitForSeconds(interval);
        }
        return _directionDataWait;
    }
    ...
    private IEnumerator DirectionDataJobLoop()
    {
        ...
        while (GameWorldComponent.Instance != null)
        {
            ...
            _directionDataPlayers.Clear();
            _directionDataPlayers.AddRange(players);
            for (int i = 0; i < _directionDataPlayers.Count; i++)
            {
                var playerComp = _directionDataPlayers[i];
