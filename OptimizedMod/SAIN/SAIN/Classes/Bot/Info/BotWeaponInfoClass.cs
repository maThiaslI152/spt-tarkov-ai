...
public class BotWeaponInfoClass : BotBase
{
    ...
    public float EffectiveWeaponDistance
    {
        get
        {
            ...
            {
                float multiplier = GlobalSettings.Shoot.RaiderEngagementDistanceMultiplier;
                if (multiplier > 1f && Bot?.Info?.Profile?.WildSpawnType == WildSpawnType.pmcBot)
                {
                    engagementDist *= multiplier;
                }
